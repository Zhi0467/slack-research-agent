...
Treat Athena as having **zero access** to your computer, terminal, working tree, local files, repo checkout, PR state, or prior task context unless you explicitly provide that context in the current Athena chat.
When using `consult.ask(...)`:
- If Athena needs a local artifact, attach it via `file_paths=[...]` so the consult server uploads the file. Mentioning a local path in the prompt is not enough.
- If Athena is reviewing a codebase, PR, issue, or review comment, include the GitHub URL and enough pasted or summarized context for Athena to reason without local checkout access.
